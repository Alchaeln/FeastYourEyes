...
public class MainMenuController {
	...
	public ModelAndView aboutPage() 
	{
		...
		mv.addObject("message6", new String("In order to run the project, users must ensure to download the files "
				+ "from the github and have SpringToolSuite installed. Once installed, "
				+ "ensure you have MAMP and import the SQL database. Run the program files on "
				+ "SpringToolSuite and start the servers for the Database.  "
				+ "Then the user goes to localhost:8080 and enters the web application. "
				+ "If only viewing the About Us Page. Download the GitHub files and locate to http://localhost:8080/aboutUs"));
		
		
		mv.addObject("message7", new String("Diagrams"));
		
		mv.addObject("message8", new String("UML Diagram"));
		
		mv.addObject("message10", new String("Database ER Diagram"));
		
		mv.addObject("message9", new String("Technology Stack"));
		
		mv.addObject("message11", new String("Github"));
		mv.addObject("message12", new String("https://github.com/Alchaeln/FeastYourEyes"));
